...
from src.domain.value_objects import Utterance, TimestampRange, Word, ConfidenceScore, LanguageTag

def test_sentence_segmentation_enricher_splits_long_monologue():
    # Arrange
    # A long monologue (20 seconds) with two sentences
    words = [
        Word("Hello", TimestampRange(timedelta(seconds=0), timedelta(seconds=1)), ConfidenceScore(1.0)),
        Word("world.", TimestampRange(timedelta(seconds=1), timedelta(seconds=2)), ConfidenceScore(1.0)),
        Word("This", TimestampRange(timedelta(seconds=11), timedelta(seconds=12)), ConfidenceScore(1.0)),
        Word("is", TimestampRange(timedelta(seconds=12), timedelta(seconds=13)), ConfidenceScore(1.0)),
        Word("long.", TimestampRange(timedelta(seconds=13), timedelta(seconds=14)), ConfidenceScore(1.0)),
    ]
