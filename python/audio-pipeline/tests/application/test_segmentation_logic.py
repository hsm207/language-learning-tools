...
from src.domain.value_objects import Utterance, Word, TimestampRange, ConfidenceScore, LanguageTag
...
def raw_utterances():
    ...
    for segment in data.get("transcription", []):
        ...
        for token in segment.get("tokens", []):
            ...
            words.append(Word(
                text=t_text,
                timestamp=TimestampRange(timedelta(milliseconds=t_start), timedelta(milliseconds=t_end)),
        ...
        if words:
            ...
            utterances.append(Utterance(
                timestamp=TimestampRange(timedelta(milliseconds=seg_start), timedelta(milliseconds=seg_end)),
